...
const SDPCandidateType_1 = __importDefault(require("./SDPCandidateType"));
const SDPMediaSection_1 = __importDefault(require("./SDPMediaSection"));
const VideoCodecCapability_1 = __importDefault(require("./VideoCodecCapability"));
/**
...
class SDP {
     ...
     */
    withDependencyDescriptorRtpHeaderExtension(previousSdp) {
    ...
    withoutDependencyDescriptorRtpHeaderExtension() {
        const srcLines = SDP.splitLines(this.sdp).filter(line => !line.includes(SDP.dependencyDescriptorRtpHeaderExtensionUrl));
        return new SDP(srcLines.join(SDP.CRLF) + SDP.CRLF);
    }
    withRtpHeaderExtension(previousSdp, url) {
    ...
    sectionWithCodecPreferences(section, preferences) {
        ...
        lines.forEach(line => {
            ...
            const payloadTypeString = payloadMatch[1];
            for (const preference of preferences) {
                ...
                if (preference.codecCapability.sdpFmtpLine !== undefined) {
                    ...
                    for (const prospectiveFmtpLine of lines) {
                        ...
                        hasVp9ProfileId || (hasVp9ProfileId = prospectiveFmtpLine.startsWith(`a=fmtp:${payloadTypeString} profile-id=`));
                    }
                    if (preference.equals(VideoCodecCapability_1.default.vp9Profile0()) && !hasVp9ProfileId) {
                        codecMatches = true;
                    }
                }
                else {
        ...
        const payloadTypesToRemove = new Set(payloadTypeToCodec.keys());
    ...
    prioritizedSendVideoCodecCapabilities() {
        ...
        const capabilities = payloadTypes
            ...
            return (capability !== undefined && ['VP8', 'H264', 'VP9', 'AV1'].includes(capability.codecName));
        });
        return capabilities;
    }
    videoCodecCapabilityForPayloadType(sectionLines, payloadType) {
        ...
        if (codecName !== undefined) {
            return new VideoCodecCapability_1.default(codecName, {
    ...
    static findAudioPayloadType(codecName, sections) {
     ...
     */
    withStartingVideoSendBitrate(bitrateKbps) {
        const sections = SDP.splitSections(this.sdp);
        const cameraLineIndex = SDP.findActiveCameraSection(sections);
        if (cameraLineIndex === -1) {
            return this;
        }
        const srcLines = SDP.splitLines(sections[cameraLineIndex]);
        const dstLines = [];
        for (const line of srcLines) {
...
SDP.dependencyDescriptorRtpHeaderExtensionUrl = `https://aomediacodec.github.io/av1-rtp-spec/#dependency-descriptor-rtp-header-extension`;
SDP.videoLayersAllocationRtpHeaderExtensionUrl = `http://www.webrtc.org/experiments/rtp-hdrext/video-layers-allocation00`;
